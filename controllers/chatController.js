...
export function renameGroup(req,res,next){
  ...
  Chat.findOneAndUpdate({_id:chatId},{chatName},{new:true}).then((chat)=>{
...
export function addToGroup(req,res,next){
  ...
  Chat.findOneAndUpdate({_id:chatId},{$push:{user:userId}},{new:true}).then((response)=>{
...
export function removeFromGroup(req,res,next){
  const {chatId,userId}=req.body;
  Chat.findOneAndUpdate({_id:chatId},{$pull:{user:userId}},{new:true}).then((response)=>{
    res.status(200).send({message: response})
  }).catch((error) => {
    res.status(error).send({message: error.message})
    next(error);
